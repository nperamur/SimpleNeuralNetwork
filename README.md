...
Each Connection has a randomly generated weight and pointers to the previous and next nodes. This forms a graph structure which is the foundation for my implementation of this Neural Network.

**Prediction**:
During Forward Propagation, it simply does a BFS graph traversal using a unique queue (so we do not visit the same node multiple times), doing a weighted sum of all the weights and biases of the previous layer. This accumulation is then stored within the Neuron object itself.
As for the activation function, this program uses sigmoid which maps the value put into the function between 0 and 1. When we reach the output layer, whichever Neuron that has the highest output value wins 
...
When I first tested it, the Neural Network was biased towards very frequently predicting a win due to the fact that Madrid win a lot but it did not satisfy my expectations of  utilizing various factors. To make up for this discrepancy, I adjusted my training method so that it would be punished more than usual for getting a loss incorrect by training it more on losses and draws
than the natural occurrence in the original dataset. Although its accuracy technically did not improve due to the unpredictable nature of sports, I would consider this a win because I noticed how it would develop patterns
like how Real Madrid tends to win in the UCL at home more frequently than away, and how Madrid is more likely to beat easier opposition than tough teams.

### Limitations
I developed this Neural Network from scratch in order to learn the fundementals on how these models work. As a result, I used this graph-based CPU approach because it was the best way for me to picture and understand how they work.
